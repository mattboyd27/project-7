...
Our methodology is to use available data to predict the probability a pitch will be called a strike. We can compare this probability to the decision made by the umpire to assess how well the catcher performed catcher framing. If a low probability of strike pitch was called a strike, we give the catcher a 'credit' of 1 - p(strike), and vice versa for if it is called a ball. We will run this procedure for all of the pitches grouped by each catcher. From here we can find out which catchers during the 2021 season excelled at catcher framing, by comparing how much credit they got overall.
